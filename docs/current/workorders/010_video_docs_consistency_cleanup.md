...
| `updateq_osc_coverage_snapshot.md` | Generated registry table shows `Video` real writes as `0`, while Phase 4C has one specialized runtime-validated exception. | Clarify that the table is registry baseline coverage and specialized token gates are documented as exceptions below the table. |
