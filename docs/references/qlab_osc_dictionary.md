...
```text
/cue/x/property/+ {delta} adds {delta} to the current value.
/cue/x/property/- {delta} subtracts {delta} from the current value.
```
Starting with QLab 5.5, you can optionally send {delta} as part of the OSC address, instead of as an argument.
...
/cue/10/preWait/+/1
```
These two messages are equivalent. Sending either one increases the preWait of cue 10 by one second.

Starting with QLab 5.5, OSC messages which accept +/- syntax to move through a finite list of options will "roll over" if you add or subtract beyond the scope of the list.

For example, /cue/{cue_number}/continueMode/+ 1 will cycle from "Auto-follow" back to "Do not continue", etc.
...
Using "live" messages also does not cause QLab to register an undo-able action. Naturally, that means that a change that you make in the workspace using a "live" OSC message cannot be reversed by choosing Undo from the Edit menu or typing ⌘Z. Another, perhaps less obvious consequence of this is that sending lots and lots of "live" OSC messages to QLab requires less processing power, because QLab doesn't have to add entries into the list of undo-able items. Therefore, if you're planning to send large amounts of OSC data into QLab at high speeds, consider using "live" messages as much as possible.
...
```text
/workspace/{id}/liveFadePreview {boolean}
```
view	edit	control	query
read	read/write	read/write	
...
```text
/workspace/{id}/playhead/none
/workspace/{id}/playbackPosition/none
```
view	edit	control	query
Unset the playhead (also called the playback position) of the active cue list.
...
```text
/settings/audio/map/{map_name}
/settings/audio/mapID/{map_ID}
```
view	edit	control	query	+/-?
read only	read only	read only		
Return a array containing all the details of the specified audio map. These details include the width, height, and unique ID of the map, and nested arrays for the filter, marks, and map objects in the map. Those nested arrays contain all the attributes of each item like spread, name, unique ID, etc. The attributes in each array will differ, of course, since filters, marks, and map objects all have different attributes.

```text
/settings/audio/map/{map_name}/filters
/settings/audio/mapID/{map_ID}/filters
/settings/audio/map/{map_name}/marks
/settings/audio/mapID/{map_ID}/marks
/settings/audio/map/{map_name}/objects
/settings/audio/mapID/{map_ID}/objects
/settings/audio/map/{map_name}/size
/settings/audio/mapID/{map_ID}/size
/settings/audio/map/{map_name}/size/height
/settings/audio/mapID/{map_ID}/size/height
/settings/audio/map/{map_name}/size/width
/settings/audio/mapID/{map_ID}/size/width
/settings/audio/map/{map_name}/uniqueID
/settings/audio/mapID/{map_ID}/uniqueID
```
view	edit	control	query	+/-?
read only	read only	read only		
Return a subset of the data returned by /settings/audio/map or /settings/audio/mapID.

```text
/settings/audio/map/{map_name}/filter/{name}
/settings/audio/mapID/{map_ID}/filter/{name}
/settings/audio/map/{map_name}/filterID/{id}
/settings/audio/mapID/{map_ID}/filterID/{id}
```
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified filter of the specified audio map.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

```text
/settings/audio/map/{map_name}/filter/{name}/angle {number}
/settings/audio/mapID/{map_ID}/filter/{name}/angle {number}
/settings/audio/map/{map_name}/filterID/{id}/angle {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/angle {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the angle of the specified filter in degrees away from horizontal.

Write: If number is given, set the angle of the specified filter to number. number can be any whole or decimal number from -180 to 180.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

```text
/settings/audio/map/{map_name}/filter/{name}/passthrough/{output} {boolean}
/settings/audio/mapID/{map_ID}/filter/{name}/passthrough/{output} {boolean}
/settings/audio/map/{map_name}/filterID/{id}/passthrough/{output} {boolean}
/settings/audio/mapID/{map_ID}/filterID/{id}/passthrough/{output} {boolean}
```
view	edit	control	query	+/-?
output is required and must be an integer from 1 to 128.

Read: If no argument is given, return TRUE if the specified output is set to pass through the filter or FALSE if it is not.

Write: Check or uncheck the passthrough checkbox of the specified output in the specified filter. See details on booleans at the beginning of this section.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

```text
/settings/audio/map/{map_name}/filter/{name}/passthroughs
/settings/audio/mapID/{map_ID}/filter/{name}/passthroughs
/settings/audio/map/{map_name}/filterID/{id}/passthroughs
/settings/audio/mapID/{map_ID}/filterID/{id}/passthroughs
```
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the passthrough checkboxes of the specified filter.

```text
/settings/audio/map/{map_name}/filter/{name}/name {string}
/settings/audio/mapID/{map_ID}/filter/{name}/name {string}
/settings/audio/map/{map_name}/filterID/{id}/name {string}
/settings/audio/mapID/{map_ID}/filterID/{id}/name {string}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified filter.

Write: If string is given, set the name of the specified filter to string.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

```text
/settings/audio/map/{map_name}/filter/{name}/position {x} {y}
/settings/audio/mapID/{map_ID}/filter/{name}/position {x} {y}
/settings/audio/map/{map_name}/filterID/{id}/position {x} {y}
/settings/audio/mapID/{map_ID}/filterID/{id}/position {x} {y}
/settings/audio/map/{map_name}/filter/{name}/position/live {x} {y}
/settings/audio/mapID/{map_ID}/filter/{name}/position/live {x} {y}
/settings/audio/map/{map_name}/filterID/{id}/position/live {x} {y}
/settings/audio/mapID/{map_ID}/filterID/{id}/position/live {x} {y}
```
view	edit	control	query	+/-?	live
control
Read: If no arguments are given, return the position of the specified filter.

Write: If x and y are given, set the position of the specified filter to (x,y). x and y can be any whole or decimal numbers.
...
```text
/settings/audio/map/{map_name}/filter/{name}/position/x {number}
/settings/audio/mapID/{map_ID}/filter/{name}/position/x {number}
/settings/audio/map/{map_name}/filterID/{id}/position/x {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/position/x {number}
/settings/audio/map/{map_name}/filter/{name}/position/x/live {number}
/settings/audio/mapID/{map_ID}/filter/{name}/position/x/live {number}
/settings/audio/map/{map_name}/filterID/{id}/position/x/live {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/position/x/live {number}
```
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the X-axis position of the specified filter.

Write: If number is given, set the X-axis position of the specified filter to number. number can be any whole or decimal number.
...
```text
/settings/audio/map/{map_name}/filter/{name}/position/y {number}
/settings/audio/mapID/{map_ID}/filter/{name}/position/y {number}
/settings/audio/map/{map_name}/filterID/{id}/position/y {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/position/y {number}
/settings/audio/map/{map_name}/filter/{name}/position/y/live {number}
/settings/audio/mapID/{map_ID}/filter/{name}/position/y/live {number}
/settings/audio/map/{map_name}/filterID/{id}/position/y/live {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/position/y/live {number}
```
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the Y-axis position of the specified filter.

Write: If number is given, set the Y-axis position of the specified filter to number. number can be any whole or decimal number.
...
```text
/settings/audio/map/{map_name}/filter/{name}/uniqueID
/settings/audio/mapID/{map_ID}/filter/{name}/uniqueID
```
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified filter.

```text
/settings/audio/map/{map_name}/filter/{name}/width {number}
/settings/audio/mapID/{map_ID}/filter/{name}/width {number}
/settings/audio/map/{map_name}/filterID/{id}/width {number}
/settings/audio/mapID/{map_ID}/filterID/{id}/width {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the width of the specified filter.

Write: If number is given, set the width of the specified filter to number. number can be any whole or decimal number 0 or greater, limited to half of the largest dimension of the map.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}
/settings/audio/mapID/{map_ID}/mark/{name}
/settings/audio/map/{map_name}/markID/{id}
/settings/audio/mapID/{map_ID}/markID/{id}
```
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified mark.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/gravity {number}
/settings/audio/mapID/{map_ID}/mark/{name}/gravity {number}
/settings/audio/map/{map_name}/markID/{id}/gravity {number}
/settings/audio/mapID/{map_ID}/markID/{id}/gravity {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the gravity of the specified mark.

Write: If number is given, set the gravity of the specified mark to number. number can be any whole or decimal number from 0.1 to 5.0.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/level/{outChannel} {decibel}
/settings/audio/mapID/{map_ID}/mark/{name}/level/{outChannel} {decibel}
/settings/audio/map/{map_name}/markID/{id}/level/{outChannel} {decibel}
/settings/audio/mapID/{map_ID}/markID/{id}/level/{outChannel} {decibel}
```
view	edit	control	query	+/-?
outChannel is required and must be an integer from 0 to 128. 0 is the main row.
...
```text
/settings/audio/map/{map_name}/mark/{name}/levels
/settings/audio/mapID/{map_ID}/mark/{name}/levels
/settings/audio/map/{map_name}/markID/{id}/levels
/settings/audio/mapID/{map_ID}/markID/{id}/levels
```
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the levels of the specified mark.

```text
/settings/audio/map/{map_name}/mark/{name}/name {string}
/settings/audio/mapID/{map_ID}/mark/{name}/name {string}
/settings/audio/map/{map_name}/markID/{id}/name {string}
/settings/audio/mapID/{map_ID}/markID/{id}/name {string}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified mark.

Write: If string is given, set the name of the specified mark to string.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/position {x} {y}
/settings/audio/mapID/{map_ID}/mark/{name}/position {x} {y}
/settings/audio/map/{map_name}/markID/{id}/position {x} {y}
/settings/audio/mapID/{map_ID}/markID/{id}/position {x} {y}
```
view	edit	control	query	+/-?
Read: If no arguments are given, return the position of the specified mark.

Write: If x and y are given, set the position of the specified mark to (x,y). x and y can be any whole or decimal numbers.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/position/x {number}
/settings/audio/mapID/{map_ID}/mark/{name}/position/x {number}
/settings/audio/map/{map_name}/markID/{id}/position/x {number}
/settings/audio/mapID/{map_ID}/markID/{id}/position/x {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the X-axis position of the specified mark.

Write: If number is given, set the X-axis position of the specified mark to number. number can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/position/y {number}
/settings/audio/mapID/{map_ID}/mark/{name}/position/y {number}
/settings/audio/map/{map_name}/markID/{id}/position/y {number}
/settings/audio/mapID/{map_ID}/markID/{id}/position/y {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the Y-axis position of the specified mark.

Write: If number is given, set the Y-axis position of the specified mark to number. number can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/shadow {number}
/settings/audio/mapID/{map_ID}/markID/{id}/shadow {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the shadow angle of the specified mark.

Write: If number is given, set the shadow angle of the specified mark to number. number can be any whole or decimal number from 0 to 90.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

```text
/settings/audio/map/{map_name}/mark/{name}/uniqueID
/settings/audio/mapID/{map_ID}/mark/{name}/uniqueID
```
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified mark.

```text
/settings/audio/map/{map_name}/object/{name}
/settings/audio/mapID/{map_ID}/object/{name}
/settings/audio/map/{map_name}/objectID/{id}
/settings/audio/mapID/{map_ID}/objectID/{id}
```
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified map object.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

```text
/settings/audio/map/{map_name}/object/{name}/colorName {number}
/settings/audio/mapID/{map_ID}/object/{name}/colorName {number}
/settings/audio/map/{map_name}/objectID/{id}/colorName {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/colorName {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the color of the specified map object.

Write: If string is given, set the color of the specified map object to string. Valid colors are:
...
```text
/settings/audio/map/{map_name}/object/{name}/name {string}
/settings/audio/mapID/{map_ID}/object/{name}/name {string}
/settings/audio/map/{map_name}/objectID/{id}/name {string}
/settings/audio/mapID/{map_ID}/objectID/{id}/name {string}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified map object.

Write: If string is given, set the name of the specified map object to string.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

```text
/settings/audio/map/{map_name}/object/{name}/position {x} {y}
/settings/audio/mapID/{map_ID}/object/{name}/position {x} {y}
/settings/audio/map/{map_name}/objectID/{id}/position {x} {y}
/settings/audio/mapID/{map_ID}/objectID/{id}/position {x} {y}
/settings/audio/map/{map_name}/object/{name}/position/live {x} {y}
/settings/audio/mapID/{map_ID}/object/{name}/position/live {x} {y}
/settings/audio/map/{map_name}/objectID/{id}/position/live {x} {y}
/settings/audio/mapID/{map_ID}/objectID/{id}/position/live {x} {y}
```
view	edit	control	query	+/-?	live
control
Read: If no arguments are given, return the position of the specified map object.

Write: If x and y are given, set the position of the specified map object to (x,y). x and y can be any whole or decimal numbers.
...
```text
/settings/audio/map/{map_name}/object/{name}/position/x {number}
/settings/audio/mapID/{map_ID}/object/{name}/position/x {number}
/settings/audio/map/{map_name}/objectID/{id}/position/x {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/position/x {number}
/settings/audio/map/{map_name}/object/{name}/position/x/live {number}
/settings/audio/mapID/{map_ID}/object/{name}/position/x/live {number}
/settings/audio/map/{map_name}/objectID/{id}/position/x/live {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/position/x/live {number}
```
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the X-axis position of the specified map object.

Write: If number is given, set the X-axis position of the specified map object to number. number can be any whole or decimal number.
...
```text
/settings/audio/map/{map_name}/object/{name}/position/y {number}
/settings/audio/mapID/{map_ID}/object/{name}/position/y {number}
/settings/audio/map/{map_name}/objectID/{id}/position/y {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/position/y {number}
/settings/audio/map/{map_name}/object/{name}/position/y/live {number}
/settings/audio/mapID/{map_ID}/object/{name}/position/y/live {number}
/settings/audio/map/{map_name}/objectID/{id}/position/y/live {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/position/y/live {number}
```
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the Y-axis position of the specified map object.

Write: If number is given, set the Y-axis position of the specified map object to number. number can be any whole or decimal number.
...
```text
/settings/audio/map/{map_name}/object/{name}/spread {number}
/settings/audio/mapID/{map_ID}/object/{name}/spread {number}
/settings/audio/map/{map_name}/objectID/{id}/spread {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/spread {number}
/settings/audio/map/{map_name}/object/{name}/spread/live {number}
/settings/audio/mapID/{map_ID}/object/{name}/spread/live {number}
/settings/audio/map/{map_name}/objectID/{id}/spread/live {number}
/settings/audio/mapID/{map_ID}/objectID/{id}/spread/live {number}
```
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the spread of the specified map object.

Write: If number is given, set the spread of the specified map object to number. number can be any whole or decimal number from 0 to twice the largest dimension of the map.
...
```text
/settings/audio/map/{map_name}/object/{name}/uniqueID
/settings/audio/mapID/{map_ID}/object/{name}/uniqueID
```
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified map object.

```text
/settings/audio/map/{map_name}/testObject
/settings/audio/mapID/{map_ID}/testObject
```
view	edit	control	query
read only	read only	read only	
Return a array containing the level, position, and running status of the test object in the specified map.

```text
/settings/audio/maps/testObjectFileTarget {string}
```
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the target of the audio map test object. Empty string ("") means that the cue has no file target.

Write: If string is given, set the target of the audio map test object to string. If string is "none" or empty (""), unset the file target. If string is anything else, string should be a file path and name in any of the following three forms:
...
```text
/settings/audio/map/{map_name}/testObject/level {number}
/settings/audio/mapID/{map_ID}/testObject/level {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the level of the test object in the specified map.

Write: If number is given, set the level of the test object in the specified map to number. number can be any whole or decimal number between the minimum and maximum workspace audio level.

```text
/settings/audio/map/{map_name}/testObject/position {x} {y}
/settings/audio/mapID/{map_ID}/testObject/position {x} {y}
```
view	edit	control	query	+/-?
Read: If no arguments are given, return the position of the test object in the specified map.

Write: If x and y are given, set the position of the test object in the specified map to (x,y). x and y can be any whole or decimal numbers.

```text
/settings/audio/map/{map_name}/testObject/position/x {number}
/settings/audio/mapID/{map_ID}/testObject/position/x {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the X-axis position of the test object in the specified map.

Write: If number is given, set the X-axis position of the test object in the specified map to number. number can be any whole or decimal number.

```text
/settings/audio/map/{map_name}/testObject/position/y {number}
/settings/audio/mapID/{map_ID}/testObject/position/y {number}
```
view	edit	control	query	+/-?
Read: If no argument is given, return the Y-axis position of the test object in the specified map.

Write: If number is given, set the Y-axis position of the test object in the specified map to number. number can be any whole or decimal number.

```text
/settings/audio/map/{map_name}/testObject/running {boolean}
/settings/audio/mapID/{map_ID}/testObject/running {boolean}
```
view	edit	control	query	+/-?
Read: If no argument is given, return TRUE if the test object in the specified map is running, or FALSE if it is not.

Write: Start or stop the test object in the specified map. See details on booleans at the beginning of this section.

```text
/settings/audio/map/{map_name}/testObject/start
/settings/audio/mapID/{map_ID}/testObject/start
```
view	edit	control	query
Start the test object in the specified map.

```text
/settings/audio/map/{map_name}/testObject/stop
/settings/audio/mapID/{map_ID}/testObject/stop
```
view	edit	control	query
Stop the test object in the specified map.
...
view	edit	control	query	+/-?
read only	read only	read only		
Return a array containing all the details of all the audio maps in the workspace. These details include the name, width, height, and unique ID of the map, and nested arrays for the filter, marks, and map objects in the map. Those nested arrays contain all the attributes of each item like spread, name, unique ID, etc. The attributes in each array will differ, of course, since filters, marks, and map objects all have different attributes.

```text
/settings/audio/maps/undo
```
view	edit	control	query
Undo the most recent change in the Audio Map Editor. If there is no valid action to undo, this message has no effect.

```text
/settings/audio/maps/redo
```
view	edit	control	query
Redo the most recent un-done change in the Audio Map Editor. If there is no valid action to redo, this message has no effect.
...
/cue/{cue_number}/canHaveAudioMapTargets
view	edit	control	query	+/-?
read only	read only	read only		
Return true if the specified cue can target an audio map, such as a Fade or Reset cue. Otherwise, return false.
...
Deprecated in QLab 5.5. This message works but will be removed in a future version of QLab. Use /currentCueTargetID instead.

/cue/{cue_number}/currentCueTargetID
view	edit	control	query	+/-?
read only	read only	read only		
Return the cue ID of the current target of the specified cue. Empty string ("") means that the cue has no current target.

/cue/{cue_number}/currentCueTargetNumber
view	edit	control	query	+/-?
read only	read only	read only		
Return the cue number of the current target of the specified cue. Empty string ("") means that the cue has no current target.
...
view	edit	control	query	+/-?
read only	read only	read only		
Return true if the specified cue is the next cue in a  Playlist Group. "Next" in this case means the next to begin playing, either automatically when the currently playing cue in the  Playlist Group ends, or manually if the currently playing cue in the  Playlist Group is of infinite duration.
...
/cue/{cue_number}/skipIfDisarmed {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the state of the Skip if disarmed checkbox of the specified cue.

Write: Set the state of the Skip if disarmed checkbox of the specified cue. See details on booleans at the beginning of this section.
...
Write: If the specified cue is a cue list, set the playhead (playback position). string can be:

The cue number of a cue in the list
next, which will move the playhead down by one cue.
previous, which will move the playhead up by one cue.
none, which will unset the playhead.
...
/cue/{cue_number}/playbackPosition/next
view	edit	control	query	+/-?
If the specified cue is a cue list, set the playhead in that cue list to the next cue.

/cue/{cue_number}/playhead/none
/cue/{cue_number}/playbackPosition/none
view	edit	control	query	+/-?
If the specified cue is a cue list, unset the playhead in that cue list.

/cue/{cue_number}/playhead/previous
/cue/{cue_number}/playbackPosition/previous
view	edit	control	query	+/-?
...
/cue/{cue_number}/playbackPosition/nextSequence
view	edit	control	query	+/-?
...
/cue/{cue_number}/playbackPosition/previousSequence
view	edit	control	query	+/-?
If the specified cue is a cue list, set the playhead in that cue list to the previous cue sequence.

/cue/{cue_number}/playlist/currentCue {string}
/cue/{cue_number}/playlist/currentCueID {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the cue number or ID of the current cue of the specified  Playlist Group cue.

Write: Set the current cue of the specified  Playlist Group cue to string.
...
/cue/{cue_number}/audioMap
view	edit	control	query	+/-?
read only	read only	read only		
Return a array containing all the details of the map in use by the specified cue. These details include the width, height, and unique ID of the map, and nested arrays for the filters, marks, and map objects in the map, containing all the attributes of each item like spread, name, unique ID, etc. The attributes in each array will differ, of course, since filters, marks, and map objects all have different attributes.

/cue/{cue_number}/audioMap/filters
/cue/{cue_number}/audioMap/marks
/cue/{cue_number}/audioMap/objects
/cue/{cue_number}/audioMap/size
/cue/{cue_number}/audioMap/size/height
/cue/{cue_number}/audioMap/size/width
/cue/{cue_number}/audioMap/uniqueID
view	edit	control	query	+/-?
read only	read only	read only		
Return a subset of the data returned by /cue/{cue_number}/audioMap.

/cue/{cue_number}/audioMap/filter/{name}
/cue/{cue_number}/audioMap/filterID/{id}
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified filter in the map of the specified cue.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

/cue/{cue_number}/audioMap/filter/{name}/angle {number}
/cue/{cue_number}/audioMap/filterID/{id}/angle {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the angle of the specified filter in degrees away from horizontal.
...
/cue/{cue_number}/audioMap/filter/{name}/passthrough/{output} {boolean}
/cue/{cue_number}/audioMap/filterID/{id}/passthrough/{output} {boolean}
view	edit	control	query	+/-?
output is required and must be an integer from 1 to 128.
...
/cue/{cue_number}/audioMap/filter/{name}/passthroughs
/cue/{cue_number}/audioMap/filterID/{id}/passthroughs
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the passthrough checkboxes of the specified filter.

/cue/{cue_number}/audioMap/filter/{name}/name {string}
/cue/{cue_number}/audioMap/filterID/{id}/name {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified filter.
...
/cue/{cue_number}/audioMap/filter/{name}/position {x} {y}
/cue/{cue_number}/audioMap/filterID/{id}/position {x} {y}
view	edit	control	query	+/-?
Read: If no arguments are given, return the position of the specified filter.

Write: If x and y are given, set the position of the specified filter to (x,y). x and y can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /filter/*B) and bracketed ranges /(e.g. /filter/[2-6]).

/cue/{cue_number}/audioMap/filter/{name}/position/x {number}
/cue/{cue_number}/audioMap/filterID/{id}/position/x {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the X-axis position of the specified filter.
...
/cue/{cue_number}/audioMap/filter/{name}/position/y {number}
/cue/{cue_number}/audioMap/filterID/{id}/position/y {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the Y-axis position of the specified filter.
...
/cue/{cue_number}/audioMap/filter/{name}/uniqueID
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified filter.

/cue/{cue_number}/audioMap/filter/{name}/width {number}
/cue/{cue_number}/audioMap/filterID/{id}/width {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the width of the specified filter.
...
/cue/{cue_number}/audioMap/mark/{name}
/cue/{cue_number}/audioMap/markID/{id}
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified mark in the map of the audio output patch of the specified cue.

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

/cue/{cue_number}/audioMap/mark/{name}/gravity {number}
/cue/{cue_number}/audioMap/markID/{id}/gravity {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the gravity of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/level/{outChannel} {decibel}
/cue/{cue_number}/audioMap/markID/{id}/level/{outChannel} {decibel}
view	edit	control	query	+/-?
outChannel is required and must be an integer from 0 to 128.

Read: If no decibel is given, return the volume level of the specified output.

Write: If decibel is given, set the level of the specified output to decibel. decibel can be any number, decimals allowed, or a string such as -inf. If decibel is a string (any string, in fact) QLab will set the level to -inf,

{name} can include OSC wildcard patterns such as asterisks (e.g. /mark/*B) and bracketed ranges /(e.g. /mark/[2-6]).

/cue/{cue_number}/audioMap/mark/{name}/levels
/cue/{cue_number}/audioMap/markID/{id}/levels
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the levels of the specified mark.

/cue/{cue_number}/audioMap/mark/{name}/name {string}
/cue/{cue_number}/audioMap/markID/{id}/name {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/position {x} {y}
/cue/{cue_number}/audioMap/markID/{id}/position {x} {y}
view	edit	control	query	+/-?
Read: If no arguments are given, return the position of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/position/x {number}
/cue/{cue_number}/audioMap/markID/{id}/position/x {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the X-axis position of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/position/y {number}
/cue/{cue_number}/audioMap/markID/{id}/position/y {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the Y-axis position of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/shadow {number}
/cue/{cue_number}/audioMap/markID/{id}/shadow {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the shadow angle of the specified mark.
...
/cue/{cue_number}/audioMap/mark/{name}/uniqueID
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified mark.

/cue/{cue_number}/audioMap/object/{name}
/cue/{cue_number}/audioMap/objectID/{id}
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the attributes of the specified map object in the map of the specified cue.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

/cue/{cue_number}/audioMap/object/{name}/colorName {number}
/cue/{cue_number}/audioMap/objectID/{id}/colorName {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the color of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/name {string}
/cue/{cue_number}/audioMap/objectID/{id}/name {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/position {x} {y}
/cue/{cue_number}/audioMap/object/{name}/position/live {x} {y}
/cue/{cue_number}/audioMap/objectID/{id}/position {x} {y}
/cue/{cue_number}/audioMap/objectID/{id}/position/live {x} {y}
view	edit	control	query	+/-?	live
control
Read: If no arguments are given, return the position of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/position/x {number}
/cue/{cue_number}/audioMap/object/{name}/position/x/live {number}
/cue/{cue_number}/audioMap/objectID/{id}/position/x {number}
/cue/{cue_number}/audioMap/objectID/{id}/position/x/live {number}
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the X-axis position of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/position/y {number}
/cue/{cue_number}/audioMap/object/{name}/position/y/live {number}
/cue/{cue_number}/audioMap/objectID/{id}/position/y {number}
/cue/{cue_number}/audioMap/objectID/{id}/position/y/live {number}
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the Y-axis position of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/spread {number}
/cue/{cue_number}/audioMap/object/{name}/spread/live {number}
/cue/{cue_number}/audioMap/objectID/{id}/spread {number}
/cue/{cue_number}/audioMap/objectID/{id}/spread/live {number}
view	edit	control	query	+/-?	live
control
Read: If no argument is given, return the spread of the specified map object.
...
/cue/{cue_number}/audioMap/object/{name}/uniqueID
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified map object.

/cue/{cue_number}/audioMapID {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the map ID of the audio map currently in use by the specified cue. Empty string ("") means that the cue is not using an audio map.

Write: If string is given and matches the map ID of an audio map in the workspace, set the audio map of the specified cue to that map. If string is "none" or empty (""), set the specified cue to use no map.

/cue/{cue_number}/audioMapName {string}
view	edit	control	query	+/-?
Read: If no argument is given, return the name of the audio map currently in use by the specified cue. Empty string ("") means that the cue is not using an audio map.

Write: If string is given and matches the name of an audio map in the workspace, set the audio map of the specified cue to that map. If string is "none" or empty (""), set the specified cue to use no map.

/cue/{cue_number}/audioMapNumber {number}
view	edit	control	query	+/-?
Read: If no argument is given, return the index of the audio map currently in use by the specified cue. 0 means that the cue is not using an audio map.

Write: If number is given, set the audio map of the specified cue to that map, according to the order of maps as listed in Workspace Settings → Audio → Audio Maps. If number is 0, set the specified cue to use no map.
...
Starting with QLab 5.5, {output} can include OSC wildcard patterns such as asterisks (e.g. /solo "*L") and bracketed ranges (e.g. /solo [2-6]).
...
/cue/{cue_number}/level/{inChannel}/{outChannel} {decibel}
/cue/{cue_number}/level/{inChannel}/{outChannel}/live {decibel}
view	edit	control	query	+/-?	live
read	read/write	read			control
inChannel is required and must be an integer from 0 to 24. 0 is the main column.
...
/cue/{cue_number}/levels/live
view	edit	control	query	+/-?	live
read only	read only	read only			view
Return all the audio levels currently available in the Levels tab of the specified cue's inspector. The levels are returned as an array of arrays, like so:
...
/cue/{cue_number}/liveAverageObjectLevel/{name} {low} {high}
/cue/{cue_number}/liveAverageObjectIDLevel/{id} {low} {high}
/cue/{cue_number}/liveAverageObjectLevel/{name}/{low}/{high}
/cue/{cue_number}/liveAverageObjectIDLevel/{id}/{low}/{high}
view	edit	control	query	+/-?
read only	read only	read only		
Return the RMS level of the specified object.

low and high are optional numerical values. If they are present, they will cause QLab to re-scale the output of this message to a range of values from low to high.

Examples:
/cue/1/liveAverageObjectLevel/A 0 100 will return the level of the object named "A" of cue 1 on a scale of 0 (silent) to 100 (as loud as possible).
/cue/10/liveAverageObjectLevel/B/-20/20 will return the level of the object named "B" of cue 10 on a scale of -20 (silent) to 20 (as loud as possible).
...
/cue/{cue_number}/mute/channel/{output} {boolean}
view	edit	control	query	+/-?
read	read/write	read		
...
/cue/{cue_number}/mute/object/{name} {boolean}
/cue/{cue_number}/mute/objectID/{id} {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return TRUE if the cue output of the specified cue is muted, or FALSE if it is not.

Write: Mute or unmute the specified object of the specified cue. See details on booleans at the beginning of this section..

/cue/{cue_number}/mute/clear
view	edit	control	query	+/-?
read/write			
Un-mute all cue outputs and all objects of the specified cue.

/cue/{cue_number}/mute/channel/clear
view	edit	control	query	+/-?
read/write			
Un-mute all cue outputs of the specified cue.

/cue/{cue_number}/mute/object/clear
view	edit	control	query	+/-?
read/write			
Un-mute all objects of the specified cue.

/cue/{cue_number}/muteChannels
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the cue outputs that are muted.

/cue/{cue_number}/muteObjects
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the objects that are muted.
...
/cue/{cue_number}/object/{name}
/cue/{cue_number}/objectID/{id}
view	edit	control	query	+/-?
read only	read only	read only		
Return an array containing all the details of the specified object in the specified cue. These details include color, name, position, spread, and unique ID of the object.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

/cue/{cue_number}/object/{name}/colorName {string}
/cue/{cue_number}/objectID/{id}/colorName {string}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the color of the specified object in the specified cue.

Write: If string is given, set the color of the specified object in the specified cue. string can be any valid cue color.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

/cue/{cue_number}/object/{name}/name {string}
/cue/{cue_number}/objectID/{id}/name {string}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the name of the specified object in the specified cue.

Write: If string is given, set the name of the specified object in the specified cue. string can be any text, but if you plan to refer to the object using OSC it must not include spaces or any reserved character such as \, *, ?, etc.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

/cue/{cue_number}/object/{name}/position {x} {y}
/cue/{cue_number}/object/{name}/position/live {x} {y}
/cue/{cue_number}/objectID/{id}/position {x} {y}
/cue/{cue_number}/objectID/{id}/position/live {x} {y}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no arguments are given, return the position of the specified object in the specified cue.

Write: If x and y are given, set the position of the specified object in the specified cue to (x,y). x and y can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

Note: If using both increment/decrement and live at the same time, the /live part of the address must come last, after the /+ or /- part.

/cue/{cue_number}/object/{name}/position/x {number}
/cue/{cue_number}/object/{name}/position/x/live {number}
/cue/{cue_number}/objectID/{id}/position/x {number}
/cue/{cue_number}/objectID/{id}/position/x/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the X-axis position of the specified object in the specified cue.

Write: If number is given, set the X-axis position of the specified object in the specified cue to number. number can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

Note: If using both increment/decrement and live at the same time, the /live part of the address must come last, after the /+ or /- part.

/cue/{cue_number}/object/{name}/position/y {number}
/cue/{cue_number}/object/{name}/position/y/live {number}
/cue/{cue_number}/objectID/{id}/position/y {number}
/cue/{cue_number}/objectID/{id}/position/y/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the Y-axis position of the specified object in the specified cue.

Write: If number is given, set the Y-axis position of the specified object in the specified cue to number. number can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

/cue/{cue_number}/object/{name}/spread {number}
/cue/{cue_number}/object/{name}/spread/live {number}
/cue/{cue_number}/objectID/{id}/spread {number}
/cue/{cue_number}/objectID/{id}/spread/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the spread of the specified object in the specified cue.

Write: If number is given, set the spread of the specified object in the specified cue to number. number can be any whole or decimal number.

{name} can include OSC wildcard patterns such as asterisks (e.g. /object/*B) and bracketed ranges /(e.g. /object/[2-6]).

Note: If using both increment/decrement and live at the same time, the /live part of the address must come last, after the /+ or /- part.

/cue/{cue_number}/object/{name}/uniqueID
/cue/{cue_number}/objectID/{id}/uniqueID
view	edit	control	query	+/-?
read only	read only	read only		
Return the unique ID of the specified object in the specified cue.

/cue/{cue_number}/objects
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of dictionaries containing all the details of all the objects in the specified cue. These details include color, name, position, spread, and unique ID.

/cue/{cue_number}/objectLevel/{row}/{name} {decibel}
/cue/{cue_number}/objectLevel/{row}/{name}/live {decibel}
/cue/{cue_number}/objectIDLevel/{row}/{id} {decibel}
/cue/{cue_number}/objectIDLevel/{row}/{id}/live {decibel}
view	edit	control	query	+/-?	live
read	read/write	read			control
row is required and must be an integer from 0 to 24. 0 is the main row.

name or id is required and must be a string that matches the name or ID of an object in the specified cue or a map object visible to the selected cue.
...
/cue/{cue_number}/objectLevels
/cue/{cue_number}/objectLevels/live
view	edit	control	query	+/-?	live
read only	read only	read only			view
Return all the audio levels currently available in the Objects tab of the specified cue's inspector. The levels are returned as an array of arrays, like so:

javascript

[
  [ main level, object 1 level, object 2 level, ...],
  [ input 1 main level, input 1/object 1 level, input 1/object 2 level, ...],
  [ ... ]
]
[ ... ] represents a second row in the crosspoint matrix, which is present if and only if the specified cue has at least two audio input channels. There could be as many as 24 rows.
...
/cue/{cue_number}/sliderLevel/live {channel} {decibel}
/cue/{cue_number}/sliderLevel/{channel} {decibel}
/cue/{cue_number}/sliderLevel/{channel}/live {decibel}
view	edit	control	query	+/-?
read	read/write	read		
...
/cue/{cue_number}/sliderLevels/live
view	edit	control	query	+/-?	live
read only	read only	read only			view
Return an array of the output levels of the specified cue, including the main level.
...
/cue/{cue_number}/solo/object/{name} {boolean}
/cue/{cue_number}/solo/objectID/{id} {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return TRUE if the cue output of the specified cue is soloed, or FALSE if it is not.

Write: Solo or un-solo the specified object of the specified cue. See details on booleans at the beginning of this section.

/cue/{cue_number}/solo/clear
view	edit	control	query	+/-?
read/write			
Un-solo all cue outputs and objects of the specified cue.

/cue/{cue_number}/solo/channel/clear
view	edit	control	query	+/-?
read/write			
Un-solo all cue outputs of the specified cue.

/cue/{cue_number}/solo/object/clear
view	edit	control	query	+/-?
read/write			
Un-solo all objects of the specified cue.

/cue/{cue_number}/soloChannels
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the cue outputs that are soloed.

/cue/{cue_number}/soloObjects
view	edit	control	query	+/-?
read only	read only	read only		
Return an array of the objects that are soloed.
...
Mic cue messages
Messages specific to Mic cues. Mic cues also respond to any Audio cue messages that pertain to the I/O, Levels, and Objects tabs of the inspector.
...
/cue/{cue_number}/stage/region/{name}
/cue/{cue_number}/stage/regionID/{id}
/cue/{cue_number}/stage/regionIndex/{index}
view	edit	control	query
read only	read only	read only	
Return a JSON dictionary containing all available information of the specified region of the stage that the specified cue is assigned to. The exact contents of this dictionary can vary widely, but generally includes information such as the size and gamma of edge blend zones, control point positions and link information, and so on.

/cue/{cue_number}/stage/region/{name}/bounds {x} {y} {width} {height}
/cue/{cue_number}/stage/regionID/{id}/bounds {x} {y} {width} {height}
/cue/{cue_number}/stage/regionIndex/{index}/bounds {x} {y} {width} {height}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no arguments are given, return the lower left corner coordinates, the width, and the height of the specified region.

Write: If all four arguments are given and are valid, set the position and size of the specified region. All four arguments must be numbers, decimals allowed, and taken together they should describe a location and size that is within the boundaries of the stage.

/cue/{cue_number}/stage/region/{name}/bounds/origin {x} {y}
/cue/{cue_number}/stage/regionID/{id}/bounds/origin {x} {y}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/origin {x} {y}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no arguments are given, return the lower left corner coordinates of the specified region.

Write: If both arguments are given and are valid, set the position of the specified region. Both arguments must be numbers, decimals allowed, and should describe a location that is within the boundaries of the stage.

/cue/{cue_number}/stage/region/{name}/bounds/origin/x {number}
/cue/{cue_number}/stage/regionID/{id}/bounds/origin/x {number}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/origin/x {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the X-axis coordinate of the lower left corner of the specified region.

Write: If number is given, set the X-axis position of the specified region.

/cue/{cue_number}/stage/region/{name}/bounds/origin/y {number}
/cue/{cue_number}/stage/regionID/{id}/bounds/origin/y {number}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/origin/y {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the Y-axis coordinate of the lower left corner of the specified region.

Write: If number is given, set the Y-axis position of the specified region.

/cue/{cue_number}/stage/region/{name}/bounds/size {width} {height}
/cue/{cue_number}/stage/regionID/{id}/bounds/size {width} {height}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/size {width} {height}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no arguments are given, return the size of the specified region.

Write: If both arguments are given and are valid, set the position of the specified region. Both arguments must be numbers, decimals allowed, and should describe a size that is within the boundaries of the stage.

/cue/{cue_number}/stage/region/{name}/bounds/size/height {number}
/cue/{cue_number}/stage/regionID/{id}/bounds/size/height {number}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/size/height {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the height of the specified region.

Write: If number is given, set the height of the specified region.

/cue/{cue_number}/stage/region/{name}/bounds/size/width {number}
/cue/{cue_number}/stage/regionID/{id}/bounds/size/width {number}
/cue/{cue_number}/stage/regionIndex/{index}/bounds/size/width {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the width of the specified region.

Write: If number is given, set the width of the specified region.

/cue/{cue_number}/stage/region/{name}/enableGrid {boolean}
/cue/{cue_number}/stage/regionID/{id}/enableGrid {boolean}
/cue/{cue_number}/stage/regionIndex/{index}/enableGrid {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return true if the grid of the specified region is enabled (visible), and false if it is not.

Write: If boolean is given, enable or disable the grid of the specified region. See details on booleans at the beginning of this section.

/cue/{cue_number}/stage/region/{name}/enableGuide {boolean}
/cue/{cue_number}/stage/regionID/{id}/enableGuide {boolean}
/cue/{cue_number}/stage/regionIndex/{index}/enableGuide {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return true if the guides of the route assigned to the specified region are enabled (visible), and false if they are not.

Write: If boolean is given, enable or disable the guides of the route assigned to the specified region. See details on booleans at the beginning of this section.

/cue/{cue_number}/stage/region/{name}/moveBy {x} {y}
/cue/{cue_number}/stage/regionID/{id}/moveBy {x} {y}
/cue/{cue_number}/stage/regionIndex/{index}/moveBy {x} {y}
view	edit	control	query	+/-?
Move the specified region. x and y can be any number, decimals allowed.

/cue/{cue_number}/stage/region/{name}/name
/cue/{cue_number}/stage/regionID/{id}/name
/cue/{cue_number}/stage/regionIndex/{index}/name
view	edit	control	query
read only	read only	read only	
Return the name of the specified region of the stage that the specified cue is assigned to.

/cue/{cue_number}/stage/region/{name}/resetControlPoints
/cue/{cue_number}/stage/regionID/{id}/resetControlPoints
/cue/{cue_number}/stage/regionIndex/{index}/resetControlPoints
view	edit	control	query	+/-?
Reset all warping control points of the specified region to their default positions.

/cue/{cue_number}/stage/region/{name}/uniqueID
/cue/{cue_number}/stage/regionID/{id}/uniqueID
/cue/{cue_number}/stage/regionIndex/{index}/uniqueID
view	edit	control	query
read only	read only	read only	
Return the ID of the specified region of the stage that the specified cue is assigned to.

/cue/{cue_number}/stage/regions
view	edit	control	query
read only	read only	read only	
Return a JSON dictionary describing the regions of the stage in use by the specified cue.
...
/cue/{cue_number}/videoEffects/add {name}
view	edit	control	query	+/-?
...
/cue/{cue_number}/videoEffects/insert {name} {index}
view	edit	control	query	+/-?
...
/cue/{cue_number}/videoEffect/{name}/delete
view	edit	control	query	+/-?
Remove the video effect named name from the specified cue. If there is no video effect of the specified name in use by the cue, this message has no effect. If there is more that one video effect with the same name in use by the cue, this command addresses the first instance of the video effect.

/cue/{cue_number}/videoEffectIndex/{index}/delete
view	edit	control	query	+/-?
...
/cue/{cue_number}/videoEffect/{name}/enabled {boolean}
view	edit	control	query	+/-?
read	read/write	read		
...
/cue/{cue_number}/videoEffectIndex/{index}/enabled {boolean}
view	edit	control	query	+/-?
read	read/write	read		
...
/cue/{cue_number}/videoEffect/{name}/move {newIndex}
view	edit	control	query	+/-?
...
/cue/{cue_number}/videoEffectIndex/{index}/move {newIndex}
view	edit	control	query	+/-?
...
/cue/{cue_number}/videoEffect/{name}/parameter/{parameterKey} {setting}
/cue/{cue_number}/videoEffect/{name}/parameter/{parameterKey}/live {setting}
view	edit	control	query	+/-?	live
read	read/write	read			control
...
/cue/{cue_number}/videoEffectIndex/{index}/parameter/{parameterKey}/live {setting}
view	edit	control	query	+/-?	live
read	read/write	read			control
...
/cue/{cue_number}/videoEffect/{name}/parameters {json_string}
/cue/{cue_number}/videoEffect/{name}/parameters/live {json_string}
view	edit	control	query	+/-?	live
read	read/write	read			control
...
/cue/{cue_number}/videoEffectIndex/{index}/parameters {json_string}
/cue/{cue_number}/videoEffectIndex/{index}/parameters/live {json_string}
view	edit	control	query	+/-?	live
read	read/write	read			control
...
Camera cue messages
Messages specific to Camera cues. Camera cues also respond to Audio cue, Mic cue, and Video cue messages that pertain to the I/O, Geometry, Levels, Objects, and Video FX tabs of the inspector.
...
/cue/{cue_number}/text/format/shadowColor {red} {green} {blue} {alpha}
...
/cue/{cue_number}/text/format/shadowColor/live {red} {green} {blue} {alpha}
...
/cue/{cue_number}/text/format/shadowBlurRadius {number}
/cue/{cue_number}/text/format/shadowBlurRadius/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the blur radius of the text drop shadow of the specified cue.

Write: If number is specified, set the blur radius of the text drop shadow of the specified cue. number must be a number greater than or equal to zero.
...
/cue/{cue_number}/text/format/shadowOffset {width} {height}
/cue/{cue_number}/text/format/shadowOffset/live {width} {height}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no arguments are given, return the horizontal and vertical offset of the text drop shadow of the specified cue.

Write: If width and height are specified, set the offset of the text drop shadow of the specified cue. width and height must be numbers.

This command may also use the optional /{index}/{length} and /word/{word_index} forms as described above in the entry for /cue/{cue_number}/text/format/fontFamily, although it only works if you index the first word or character in a line. It's also important to remember that the invisible "return" character at the end of a line (created by pressing the "return" key) counts as a character.

Note: If using both increment/decrement and live at the same time, the /live part of the address must come last, after the /+ or /- part.

/cue/{cue_number}/text/format/shadowOffset/width {number}
/cue/{cue_number}/text/format/shadowOffset/width/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the horizontal offset of the text drop shadow of the specified cue.

Write: If width is specified, set the horizontal offset of the text drop shadow of the specified cue. width must be a number.

This command may also use the optional /{index}/{length} and /word/{word_index} forms as described above in the entry for /cue/{cue_number}/text/format/fontFamily, although it only works if you index the first word or character in a line. It's also important to remember that the invisible "return" character at the end of a line (created by pressing the "return" key) counts as a character.

Note: If using both increment/decrement and live at the same time, the /live part of the address must come last, after the /+ or /- part.

/cue/{cue_number}/text/format/shadowOffset/height {number}
/cue/{cue_number}/text/format/shadowOffset/height/live {number}
view	edit	control	query	+/-?	live
read	read/write	read			control
Read: If no argument is given, return the vertical offset of the text drop shadow of the specified cue.

Write: If height is specified, set the vertical offset of the text drop shadow of the specified cue. height must be a number.
...
/cue/{cue_number}/audioMapTargetID {id}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the ID of the audio map currently targeted by the specified cue.

Write: If id is given, set the target of the specified cue to the specified audio map.

/cue/{cue_number}/doLevel {row} {column} {boolean}
/cue/{cue_number}/doLevel/{row}/{column} {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no arguments are given, return all the currently active crosspoints in the Levels tab of the specified cue. The levels are returned as an array of arrays, like so: [row0Array, row1Array, row2Array, ...]
...
/cue/{cue_number}/doObjectLevel {row} {object} {boolean}
/cue/{cue_number}/doObjectIDLevel {row} {object} {boolean}
/cue/{cue_number}/doObjectLevel/{row}/{object} {boolean}
/cue/{cue_number}/doObjectIDLevel/{row}/{object} {boolean}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no arguments are given, return all the currently active crosspoints in the Objects tab of the specified cue. The levels are returned as an array of arrays, like so: [row0Array, row1Array, row2Array, ...]

If row and object are given, but not boolean, return the active state of crosspoint {row,object}.

row must be an integer from 0 to 24. 0 is the main column.

{object} must be the name or ID of an audio object in the cue.

Write: If number, row and object are all given, set the active state (i.e. yellow or grey) of crosspoint {row,object}. See details on booleans at the beginning of this section.
...
Write: If number is given, set the fade type of the specified cue. Valid types are:
...
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the ID of the patch currently targeted by the specified cue.

Write: If id is given, set the target of the specified cue to the specified patch.

/cue/{cue_number}/pathHeight {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the path height of the specified cue. If the specified cue is not a 2D fade, this message has no effect.

Write: If number is given, set the path height of the specified cue. number can be any positive number.

/cue/{cue_number}/pathWidth {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the path width of the specified cue. If the specified cue is not a 2D fade, this message has no effect.

Write: If number is given, set the path width of the specified cue. number can be any positive number.
...
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return 0 if the specified cue is set to target cues or return 1 if the specified cue is set to target patches.

Write: If number is given, set the target type of the specified cue. Valid types are:
...
Deprecated in QLab 5.5. This message works in QLab 5.5, but will be removed in a future version of QLab. Use /doLevel instead.
...
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the MIDI message type of the specified cue.
...
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the MSC timecode format of the specified cue.
...
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the MSC timecode string of the specified cue.
...
/cue/{cue_number}/endTime {string}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the end time of the specified cue.

Write: If string is given, set the end time of the specified cue to string, which must be a valid timecode string (like 2:01:44:12) or a total number of seconds (like 2344.3).

/cue/{cue_number}/framerate {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the frame rate of the specified cue.

Write: If number is given, set the frame rate of the specified cue to number. Valid frame rates are:
...
4 - 23.976 fps
5 - 24.975 fps
6 - 29.97 fps non-drop
7 - 29.97 fps drop
/cue/{cue_number}/ltcChannel {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given and the type of the specified cue is LTC, return the output channel used by the cue.

Write: If number is given and the type of the specified cue is LTC, set the output channel of the specified cue to number, which must be a whole number.

If the type of the specified cue is not LTC, this message has no effect.

/cue/{cue_number}/outputType {number}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return 0 if the specified cue outputs MTC, or 1 if the specified cue outputs LTC.

Write: If number is given, set the output type of the specified cue to number. Valid frame rates are:

0 - MTC
1 - LTC
/cue/{cue_number}/startTime {string}
view	edit	control	query	+/-?
read	read/write	read		
Read: If no argument is given, return the start time of the specified cue.

Write: If string is given, set the start time of the specified cue to string, which must be a valid timecode string (like 2:01:44:12) or a total number of seconds (like 2344.3).
...
Reset cue messages
Messages specific to Reset cues.
