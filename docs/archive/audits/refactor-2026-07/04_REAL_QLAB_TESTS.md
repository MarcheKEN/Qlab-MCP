...
QLab 5.5.10 was open on `<TEST_WORKSPACE_NAME>` (`<TEST_WORKSPACE_UUID>`) in Edit Mode. `/connect` confirmed view/edit/control. All output overrides were enabled, so the review did not play cues or exercise Audio, Video, Light, MIDI, Network, timecode or show-control output.

The dedicated inactive fixture chosen for a reversible metadata write was Memo cue `41.6`, UUID `<TEST_CUE_UUID>`, name `MCP_MOVE_RUNTIME_A`. Baseline and final `notes` were both `""`; it was healthy, armed, not loaded, not running and not paused.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `notes` | saved | `""` | `MCP_REVIEW_TEMP_20260721` | `/workspace/<TEST_WORKSPACE_UUID>/cue_id/<TEST_CUE_UUID>/notes` + marker | 1 setter; `updated_with_confirmed_timeouts`; fresh after-read confirmed marker | marker | rollback 1 setter to `""`; final `""` |
