...
(`<TEST_WORKSPACE_UUID>`) with one healthy inactive cue per
type:

- `Video` cue `<TEST_VIDEO_CUE_NAME>` (`<TEST_VIDEO_CUE_UUID>`)
- `Camera` cue `<TEST_CAMERA_CUE_NAME>` (`<TEST_CAMERA_CUE_UUID>`)
- `Text` cue `<TEST_TEXT_CUE_NAME>` (`<TEST_TEXT_CUE_UUID>`)
