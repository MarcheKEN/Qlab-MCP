"""Phase 3C Video scalar write-family helpers."""
...
OPERATION_KIND = "video_phase3c_scalar_write"
TOKEN_VERSION = 1
...
def call_structure_error(items: list[dict[str, Any]]) -> str | None:
    ...
    scalar_operation = operations[0]
    if (
        scalar_operation.get("property") not in PROPERTIES
        or scalar_operation.get("path") != scalar_operation.get("property")
    ):
        return "Phase 3C real writes allow only scale, anchor, or crop scalar properties."
    if scalar_operation.get("mode") != "saved":
...
def _decode_confirm_token(token: str) -> tuple[dict[str, Any] | None, str | None]:
    payload, error = decode_confirm_token(token, "videoScalar", TOKEN_VERSION, _TOKEN_SECRET)
    if error == "malformed":
...
def dry_run_errors(
    ...
    scalar_operation = operation(item)
    if (
        scalar_operation is None
    ...
    requested = scalar_operation["args"][0] if scalar_operation.get("args") else None
...
def annotate_operation(
    ...
    *,
    ...
    scalar_operation = operation(item)
    if scalar_operation is None or item.get("profile") not in PROFILE_TYPES:
        return []
    property_name = scalar_operation["property"]
    ...
    if not candidate:
        scalar_operation.pop("confirm_token", None)
        return []
    scalar_operation.update(
        {
            "risk_tier": "high",
    ...
    scalar_operation["confirm_token"] = _confirm_token(
        workspace_id=workspace_id,
...
def validate_real_write(
    ...
    scalar_operation = operation(item)
    property_name = scalar_operation.get("property") if scalar_operation else "video_scalar"
    if scalar_operation is None or not isinstance(before, dict):
...
def mark_real_operation(item: dict[str, Any]) -> None:
    ...
    scalar_operation.pop("planned_only_reason", None)


def label_rejection(item: dict[str, Any]) -> None:
    scalar_operation = operation(item)
    if scalar_operation is not None:
        scalar_operation["planned_only_reason"] = "video_scalar_requires_confirm_token"
