...
from qlab_mcp.cues.coverage import default_read_coverage_report
...
class QLabReaderTests(unittest.TestCase):
    ...
    def test_get_workspaces_single_flight_shares_concurrent_resolution(self) -> None:
        ...
        def read() -> None:
            barrier.wait()
            results.append(reader.get_workspaces())
        ...
        self.assertEqual(results[0]["workspaces"], results[1]["workspaces"])

    def test_workspace_resolution_timeout_is_unavailable_not_not_found(self) -> None:
        class TimeoutClient:
            config = QLabConfig(cache_ttl=0)

            def request(self, address: str, *args: Any, workspace_id: str | None = None) -> Any:
                raise OscTimeoutError("Timed out waiting for QLab reply to /workspaces")

        reader = QLabReader(TimeoutClient())  # type: ignore[arg-type]

        result = reader.get_cue_details("ws-timeout", "cue-1")

        self.assertFalse(result["ok"])
        self.assertEqual(result["status"], "error")
        self.assertFalse(result["partial"])
        self.assertEqual(result["error_code"], "workspace_unavailable")
        self.assertEqual(result["errors"]["error_code"], "workspace_unavailable")
    ...
    def test_workspace_overview_returns_bounded_first_pass_summary(self) -> None:
        ...
        self.assertEqual(result["summary"]["health_counts_status"], "partial_non_authoritative")
    ...
    def test_workspace_overview_global_count_is_not_bounded_by_preview_or_index_limits(self) -> None:
        responses = {
            "/workspaces": [{"uniqueID": "ws-1", "displayName": "demo.qlab5"}],
            "/workspace/ws-1/cueLists/shallow": [{"uniqueID": "list-1", "type": "Cue List"}],
            "/workspace/ws-1/cue/list-1/children/shallow": [],
    ...
    def test_workspace_status_settings_timeout_keeps_stable_partial_shape(self) -> None:
        responses = {
            "/workspaces": [{"uniqueID": "ws-1", "displayName": "demo.qlab5"}],
            "/workspace/ws-1/cueLists/shallow": [{"uniqueID": "list-id", "type": "Cue List", "name": "Main"}],
            "/workspace/ws-1/cue/list-id/children/shallow": [],
        ...
        with FakeQlabOscServer(responses) as server:
            reader = QLabReader(client_for(server, timeout=0.05))
    ...
    def test_workspace_settings_details_empty_patch_collection_is_successful_empty_result(self) -> None:
        responses = {
            "/workspace/ws-1/settings/midi/patchList": [],
        }
        with FakeQlabOscServer(responses) as server:
            ...
            result = reader.get_workspace_settings(
                "ws-1",
                mode="details",
                requests=[{"section": "midi", "kind": "midi_patch"}],
            )
        ...
        self.assertTrue(result["results"][0]["details"]["empty"])
    ...
    def test_workspace_settings_safe_reads_selected_sections_and_redacts_destinations(self) -> None:
        responses = {
            "/workspace/ws-1/settings/video/inputPatchList": [
                {"name": "Camera 1", "uniqueID": "input-1", "deviceName": "ATEM", "source": "HDMI"}
            ],
            "/workspace/ws-1/settings/video/routes": [
                {
                    "name": "Projector",
        ...
        self.assertEqual(result["sections"]["video"]["input_patches"][0]["uniqueID"], "input-1")
        self.assertTrue(result["sections"]["video"]["input_patches"][0]["device_present"])
        self.assertTrue(result["sections"]["video"]["input_patches"][0]["source_present"])
    ...
    def test_workspace_setting_details_invalid_kind_returns_complete_structured_error(self) -> None:
        with FakeQlabOscServer({}) as server:
            reader = QLabReader(client_for(server))

            result = reader.get_workspace_setting_details("ws-1", section="midi", kind="not_a_kind")

        self.assertFalse(result["ok"])
        self.assertEqual(result["status"], "error")
        self.assertFalse(result["partial"])
        self.assertEqual(result["error_code"], "invalid_setting_kind")
        self.assertEqual(result["received"], "not_a_kind")
        self.assertIn("midi_patch", result["allowed"])
        self.assertEqual(result["details"]["section"], "midi")
        self.assertEqual(result["details"]["kind"], "not_a_kind")
        self.assertIn("Unknown workspace setting detail kind", result["message"])
    ...
    def test_query_cues_name_contains_preserves_unicode_and_nbsp_literal_matching(self) -> None:
        emoji_id = "11111111-1111-4111-8111-111111111111"
        accents_id = "22222222-2222-4222-8222-222222222222"
        nbsp_id = "33333333-3333-4333-8333-333333333333"
        responses = {
            "/workspace/ws-1/cueLists/uniqueIDs": [emoji_id, accents_id, nbsp_id],
            f"/workspace/ws-1/cue_id/{emoji_id}/valuesForKeys": {
                "uniqueID": emoji_id,
                "number": "🤣",
                "name": "UNICODE_EMOJI_🤣",
                "displayName": "UNICODE_EMOJI_🤣",
                "type": "Memo",
            },
            f"/workspace/ws-1/cue_id/{accents_id}/valuesForKeys": {
                "uniqueID": accents_id,
                "number": "ÑÓÁ",
                "name": "UNICODE_ACCENTS_ÑÓÁ",
                "displayName": "UNICODE_ACCENTS_ÑÓÁ",
                "type": "Memo",
            },
            f"/workspace/ws-1/cue_id/{nbsp_id}/valuesForKeys": {
                "uniqueID": nbsp_id,
                "number": "NBSP",
                "name": "NBSP_A\u00a0B",
                "displayName": "NBSP_A\u00a0B",
                "type": "Memo",
            },
        }
        with FakeQlabOscServer(responses) as server:
            ...
            emoji = reader.query_cues("ws-1", "name_contains", "🤣")
            accents = reader.query_cues("ws-1", "name_contains", "ñóá")
            nbsp_literal = reader.query_cues("ws-1", "name_contains", "A\u00a0B")
            nbsp_normal_space = reader.query_cues("ws-1", "name_contains", "A B")

        self.assertEqual(emoji["matched_count"], 1)
        self.assertEqual(emoji["cues"][0]["uniqueID"], emoji_id)
        self.assertEqual(accents["matched_count"], 1)
        self.assertEqual(accents["cues"][0]["uniqueID"], accents_id)
        self.assertEqual(nbsp_literal["matched_count"], 1)
        self.assertEqual(nbsp_literal["cues"][0]["uniqueID"], nbsp_id)
        self.assertEqual(nbsp_normal_space["matched_count"], 0)
    ...
    def test_single_cue_details_unresolved_returns_error_contract(self) -> None:
        ...
        with FakeQlabOscServer(responses) as server:
            ...
            result = reader.get_cue_details("ws-1", "missing", "basic_safe")
    ...
    def test_inspector_safe_profile_reads_broader_non_sensitive_fields(self) -> None:
        ...
        self.assertEqual(result["properties"]["cueSize/width"], 1280)
        self.assertEqual(result["sections"]["type_specific"]["cueSize/height"], 720)
    ...
    def test_sanitizer_removes_internal_paths_without_redacting_media_targets(self) -> None:
        payload = {
            "source_path": "/Users/filarmonica/Documents/qlab-mcp-osc/src/qlab_mcp/cues/coverage.py",
            "message": "/Users/filarmonica/Documents/qlab-mcp-osc/src/qlab_mcp/server.py failed",
            "fileTarget": "/Users/filarmonica/Documents/qlab-mcp-osc/media/valid.wav",
        }

        result = sanitize_response(payload)

        self.assertNotIn("source_path", result)
        self.assertEqual(result["message"], REDACTED_INTERNAL_PATH)
        self.assertEqual(result["fileTarget"], "/Users/filarmonica/Documents/qlab-mcp-osc/media/valid.wav")

    def test_exhaustive_read_coverage_does_not_expose_local_source_path(self) -> None:
        report = default_read_coverage_report()

        serialized = json.dumps(report)
        self.assertNotIn("source_path", report)
        self.assertNotIn("/qlab-mcp-osc/", serialized)

    def test_compact_profiles_truncate_long_text_fields_with_metadata(self) -> None:
        long_message = "/eos/cue/" + ("1234567890" * 80)
        long_notes = "operator note " * 80
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "cue-id",
                "number": "10",
                "name": "Network",
                "displayName": "10 Network",
                "type": "Network",
                "message": long_message,
                "messageError": "",
            },
            "/workspace/ws-1/cue/11/valuesForKeys": {
                "uniqueID": "cue-11",
                "type": "Memo",
                "notes": long_notes,
            },
        ...
        with FakeQlabOscServer(responses) as server:
            ...
            auto_result = reader.get_cue_details("ws-1", "10", "auto")
            technical_result = reader.get_cue_details("ws-1", "11", "technical")
            full_sensitive_result = reader.get_cue_details("ws-1", "11", "full_sensitive")

        auto_message = auto_result["sections"]["type_specific"]["message"]
        self.assertTrue(auto_message["field_truncated"])
        self.assertEqual(auto_message["original_length"], len(long_message))
        self.assertLess(len(auto_message["value"]), len(long_message))

        technical_notes = technical_result["properties"]["notes"]
        self.assertTrue(technical_notes["field_truncated"])
        self.assertEqual(technical_notes["original_length"], len(long_notes))

        self.assertEqual(full_sensitive_result["properties"]["notes"], long_notes)
        self.assertNotIn("field_truncated", json.dumps(full_sensitive_result["properties"]["notes"]))
    ...
    def test_inspector_safe_text_includes_scalar_output_and_cue_sizes(self) -> None:
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "cue-id",
                "type": "Text",
                ...
                "text/outputSize/width": 320,
                "text/outputSize/height": 120,
                "stage/regions": [{"uniqueID": "heavy-region"}],
            }
        }
        with FakeQlabOscServer(responses) as server:
            reader = QLabReader(client_for(server))
        ...
        assert result["properties"]["cueSize/width"] == 640
        assert result["properties"]["text/outputSize/height"] == 120
        assert result["sections"]["type_specific"]["text/outputSize/width"] == 320
        assert "stage/regions" not in result["properties"]

    def test_inspector_safe_video_summary_links_stage_routes_devices_and_fx(self) -> None:
        route_a = {
            "name": "Output A",
            "uniqueID": "route-a",
            "connected": True,
            "destinationInfo": {"name": "Projector", "destinationType": "Display"},
        }
        route_b = {
            "name": "Output B",
            "uniqueID": "route-b",
            "connected": False,
            "destinationInfo": {"name": "NDI", "destinationType": "NDI"},
        }
        regions = [
            {
                "name": "A",
                "uniqueID": "region-a",
                "boundsOnStage": {"x": 0, "y": 0, "width": 960, "height": 1080},
                "meshWidth": 2,
                "meshHeight": 2,
                "warpType": 3,
                "autoEdgeBlends": True,
                ...
                "controlPoints": [{"x": 0, "y": 0}],
                "shadowControlPoints": [{"x": 1, "y": 1}],
                "meshSubregions": [{"raw": "mesh"}],
                "route": route_a,
            },
            {"name": "B", "uniqueID": "region-b", "route": route_b},
        ]
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "cue-id",
                ...
                "stageID": "stage-1",
                "stageNumber": 1,
                "stage/size": {"width": 1920, "height": 1080},
                "opacity": 0.8,
            ...
            "/workspace/ws-1/settings/video/inputPatchList": [],
            "/workspace/ws-1/settings/video/routes": [route_a, route_b],
            "/workspace/ws-1/settings/video/stages": [
                {"name": "Main", "uniqueID": "stage-1", "width": 1920, "height": 1080, "regions": regions}
            ],
            "/workspace/ws-1/settings/video/stageID/stage-1/regions": regions,
        }
        with FakeQlabOscServer(responses) as server:
            reader = QLabReader(client_for(server))
            result = reader.get_cue_details("ws-1", "10", "inspector_safe")

        summary = result["sections"]["video_summary"]
        ...
        self.assertEqual(
            summary["stage"]["regions"][0],
            {
                ...
                "route": {
                    ...
                    "device": {"name": "Projector", "type": "Display", "present": True},
                },
            },
        )
        serialized_regions = json.dumps(summary["stage"]["regions"])
        for forbidden in (
            "bounds",
            "boundsOnStage",
            "meshWidth",
            ...
            self.assertNotIn(forbidden, serialized_regions)
        self.assertEqual(summary["video_fx"]["effect_count"], 1)
        self.assertEqual(summary["video_fx"]["effects"][0]["name"], "Blur")
        self.assertIn({"code": "disconnected_route"}, summary["problems"])
        self.assertNotIn("stage", result["properties"])
        self.assertNotIn("stage/regions", result["properties"])
    ...
    def test_technical_and_exhaustive_preserve_raw_video_regions(self) -> None:
        ...
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "cue-id",
                "type": "Video",
                "stage": {"uniqueID": "stage-1", "regions": [raw_region]},
                "stage/regions": [raw_region],
            }
        }
        for profile in ("technical", "exhaustive"):
            with self.subTest(profile=profile):
                with FakeQlabOscServer(responses) as server:
                    reader = QLabReader(client_for(server))
                    result = reader.get_cue_details("ws-1", "10", profile)

                region = result["properties"]["stage/regions"][0]
                self.assertEqual(region["warpType"], 3)
                self.assertEqual(region["controlPoints"], [{"x": 0, "y": 0}])
                self.assertEqual(region["route"]["destinationInfo"]["name"], "Projector")

    def test_inspector_safe_camera_input_matching_and_deprecated_fallback(self) -> None:
        patches = [
            {"name": "Descriptive Camera", "uniqueID": "patch-1"},
        ...
        cases = [
            ...
            ({"videoInputPatchID": "patch-2"}, "disconnected", "id"),
        ]
        for index, (patch_values, status, method) in enumerate(cases, start=1):
            with self.subTest(status=status, method=method):
                cue_path = f"/workspace/ws-1/cue/{index}/valuesForKeys"
                responses = {
                    cue_path: {
                        "uniqueID": f"camera-{index}",
                        "number": str(index),
                        "type": "Camera",
                        "isBroken": False,
                        "isWarning": False,
                    ...
                    "/workspace/ws-1/settings/video/inputPatchList": patches,
                    "/workspace/ws-1/settings/video/routes": [],
                    "/workspace/ws-1/settings/video/stages": [],
                }
                with FakeQlabOscServer(responses) as server:
                    reader = QLabReader(client_for(server))
                    result = reader.get_cue_details("ws-1", str(index), "inspector_safe")

                camera = result["sections"]["video_summary"]["camera_input"]
                self.assertEqual(camera["status"], status)
                self.assertEqual(camera["match_method"], method)

    def test_inspector_safe_video_settings_failure_is_auxiliary(self) -> None:
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "camera-id",
                "number": "10",
        ...
        self.assertEqual(summary["topology_status"], "unavailable")
        self.assertEqual(summary["camera_input"]["status"], "unknown")
        self.assertFalse(summary["camera_input"]["validation_available"])
        self.assertEqual(summary["camera_input"]["reason"], "input_patch_list_unavailable")

    def test_inspector_safe_non_video_batch_does_not_read_video_settings(self) -> None:
        responses = {
            "/workspace/ws-1/cue/1/valuesForKeys": {"uniqueID": "audio-1", "type": "Audio", "name": "Audio"},
            "/workspace/ws-1/cue/2/valuesForKeys": {"uniqueID": "memo-2", "type": "Memo", "name": "Memo"},
        }
        with FakeQlabOscServer(responses) as server:
            reader = QLabReader(client_for(server))
            result = reader.get_cue_details("ws-1", ["1", "2"], "inspector_safe")

        self.assertTrue(result["ok"])
        self.assertTrue(all("video_summary" not in item["sections"] for item in result["results"]))
        self.assertFalse(any("/settings/video/" in address for address in server.received))

    def test_inspector_safe_visual_batch_reads_video_settings_once(self) -> None:
        responses = {
            "/workspace/ws-1/cue/1/valuesForKeys": {"uniqueID": "video-1", "type": "Video"},
            "/workspace/ws-1/cue/2/valuesForKeys": {"uniqueID": "text-2", "type": "Text"},
            "/workspace/ws-1/settings/video/inputPatchList": [],
            "/workspace/ws-1/settings/video/routes": [],
            "/workspace/ws-1/settings/video/stages": [],
        }
        ...
        self.assertTrue(result["ok"])
        self.assertTrue(all("video_summary" in item["sections"] for item in result["results"]))
        self.assertEqual(server.received.count("/workspace/ws-1/settings/video/inputPatchList"), 1)
        self.assertEqual(server.received.count("/workspace/ws-1/settings/video/routes"), 1)
        self.assertEqual(server.received.count("/workspace/ws-1/settings/video/stages"), 1)

    def test_text_and_camera_without_file_targets_do_not_report_file_problem(self) -> None:
        for cue_type in ("Text", "Camera"):
            with self.subTest(cue_type=cue_type):
                responses = {
                    "/workspace/ws-1/cue/10/valuesForKeys": {
                        "uniqueID": "cue-id",
                ...
                self.assertFalse(any(code.startswith("video_file_target_") for code in codes))

    def test_video_without_file_target_reports_problem(self) -> None:
        responses = {
            "/workspace/ws-1/cue/10/valuesForKeys": {
                "uniqueID": "video-id",
                "type": "Video",
    ...
    def test_video_stage_reusing_one_route_is_not_multi_output(self) -> None:
        ...
        regions = [
            {"name": "A", "uniqueID": "region-a", "route": route},
            {"name": "B", "uniqueID": "region-b", "route": route},
        ]
        responses = {
            "/workspace/ws-1/settings/video/inputPatchList": [],
            "/workspace/ws-1/settings/video/routes": [route],
            "/workspace/ws-1/settings/video/stages": [
                {"name": "Main", "uniqueID": "stage-1", "width": 1920, "height": 1080, "regions": regions}
            ],
            "/workspace/ws-1/settings/video/stageID/stage-1/regions": regions,
        ...
        with FakeQlabOscServer(responses) as server:
            reader = QLabReader(client_for(server))
            result = reader.get_workspace_settings("ws-1", sections=["video"], profile="safe")

        stage = result["sections"]["video"]["stages"][0]
        self.assertEqual(stage["region_count"], 2)
        self.assertFalse(stage["multi_output"])
        self.assertFalse(any(problem["code"] == "multi_output_stage" for problem in result["sections"]["video"]["problems"]))
